...
export async function handleGroupParticipantsUpdate(c, update, botInfo) {
    ...
    if (update.action === 'promote' && update.participants.length >= 1) {
        ...
        const message = `👋 O usuário @${userAdded.split('@')[0]} foi *ADICIONADO* ao grupo 👏🍻 *DﾑMﾑS* 💃🔥 *Dﾑ* *NIGӇԵ*💃🎶🍾🍸 por @${adminWhoAdded.split('@')[0]} em ${dateTime}.`;
        ...
        const dateTime = getFormattedDateTime();
        const message = `👋 O usuário @${userRemoved.split('@')[0]} foi *REMOVIDO* do grupo 👏🍻 *DﾑMﾑS* 💃🔥 *Dﾑ* *NIGӇԵ*💃🎶🍾🍸 por @${adminWhoRemoved.split('@')[0]} em ${dateTime}.`;
